...
type ApproverPageProps = NativeStackScreenProps<RootStackParamList, 'ApproverPage3'>;

const ApproverPage3: React.FC<ApproverPageProps> = ({ route, navigation }) => {
  ...
  const [inputType, setInputType] = useState<'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | 'remark_by_approver3' | null>(null);
  ...
  const handleInputModal = (item: AfterMaintenanceItem, type: 'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | 'remark_by_approver3') => {
  ...
  const handleDeleteInput = (item: AfterMaintenanceItem, type: 'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | 'remark_by_approver3') => {
    ...
    try {
      ...
      const updatePromises = checklistData.map(item => {
        ...
        if (item.remark_by_approver || item.checklist_id || item.part_id) {
          return fetch('http://192.168.68.56:5000/api/updateApprover3', {
            method: 'PATCH',
            headers: {
      ...
      const updatedStage = "Approver3 Stage Completed"; // Set the stage value to "Inspection Stage Completed"
  ...
  return (
    ...
    <DataTable.Title style={styles.headerText}><Text style={{ fontWeight: "bold", fontSize: 14, color: "white"}}>Remark by Approver2</Text></DataTable.Title>
          ...
          <DataTable.Title style={styles.headerText}>Remark by Approver3</DataTable.Title>
        </DataTable.Header>
             ...
             <DataTable.Cell>
              ...
              <DataTable.Cell>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                  {item.remark_by_approver3 ? (
                    <>
                      <Text style={styles.valueText}>{item.remark_by_approver2}</Text>
                      <View style={{ flexDirection: "column", alignItems: "center" }}>
                        <TouchableOpacity onPress={() => handleInputModal(item, 'remark_by_approver3')}>
                          <Text style={styles.iconText}>✏️</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => handleDeleteInput(item, 'remark_by_approver3')}>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <TextInput
              style={styles.input}
              placeholder={inputType === 'remark_by_approver3' ? 'Enter Remark_by_approver3' : ''}
              value={inputValue}
