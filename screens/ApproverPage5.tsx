...
type AfterMaintenanceItem = {
  ...
  remark_by_approver5: string;
  captured_image_after_inspection: string | null;
};

type ApproverPageProps = NativeStackScreenProps<RootStackParamList, 'ApproverPage5'>;

const ApproverPage5: React.FC<ApproverPageProps> = ({ route, navigation }) => {
  ...
  const [inputType, setInputType] = useState<'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | 'remark_by_approver3' | 'remark_by_approver4' |'remark_by_approver5' | null>(null);
  ...
  const handleInputModal = (item: AfterMaintenanceItem, type: 'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | 'remark_by_approver3' | 'remark_by_approver4'| 'remark_by_approver5') => {
  ...
  const handleDeleteInput = (item: AfterMaintenanceItem, type: 'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | 'remark_by_approver3' | 'remark_by_approver4'| 'remark_by_approver5') => {
        ...
        if (item.remark_by_approver || item.checklist_id || item.part_id) {
          return fetch('http://192.168.68.56:5000/api/updateApprover4', {
            ...
            body: JSON.stringify({
              ...
              remark_by_approver3: item.remark_by_approver3,
              remark_by_approver4: item.remark_by_approver4,
              remark_by_approver5: item.remark_by_approver5
            }),
          })
            .then(response => {
              if (!response.ok) {
  ...
  return (
    ...
    <DataTable.Title style={styles.headerText}><Text style={{ fontWeight: "bold", fontSize: 14, color: "white"}}>Remark by Approver4</Text></DataTable.Title>
          ...
          <DataTable.Title style={styles.headerText}><Text style={{ fontWeight: "bold", fontSize: 14, color: "white"}}>Remark by Approver5</Text></DataTable.Title>
        </DataTable.Header>
      ...
      <DataTable.Cell>{item.remark_by_approver2}</DataTable.Cell>
       ...
       <DataTable.Cell>{item.remark_by_approver4}</DataTable.Cell>
  
              {/* Image Capture */}
          <DataTable.Cell>
                              <View style={styles.imageCell}>
                                {item.captured_image_after_inspection ? (
                                  <Image source={{ uri: `http://192.168.68.56:8080${item.captured_image_after_inspection}` }} style={styles.imagePreview} />
                                ) : null}
                              </View>
                            </DataTable.Cell>
              ...
              <DataTable.Cell>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                  {item.remark_by_approver4 ? (
                    <>
                      <Text style={styles.valueText}>{item.remark_by_approver5}</Text>
                      <View style={{ flexDirection: "column", alignItems: "center" }}>
                        <TouchableOpacity onPress={() => handleInputModal(item, 'remark_by_approver5')}>
                          <Text style={styles.iconText}>✏️</Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => handleDeleteInput(item, 'remark_by_approver5')}>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <TextInput
              style={styles.input}
              placeholder={inputType === 'remark_by_approver5' ? 'Enter Remark_by_approver5' : ''}
              value={inputValue}
