...
type ApproverPageProps = NativeStackScreenProps<RootStackParamList, 'ApproverPage2'>;

const ApproverPage2: React.FC<ApproverPageProps> = ({ route, navigation }) => {
    ...
    const [inputType, setInputType] = useState<'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2' | null>(null);
    ...
    const handleInputModal = (item: AfterMaintenanceItem, type: 'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2') => {
    ...
    const handleDeleteInput = (item: AfterMaintenanceItem, type: 'measured_value' | 'remark' | 'after_maintenance_value' | 'remark_by_approver' | 'remark_by_approver2') => {
        ...
        try {
            ...
            const updatePromises = checklistData.map(item => {
                ...
                if (item.remark_by_approver || item.checklist_id || item.part_id) {
                    return fetch('http://192.168.68.56:5000/api/updateApprover2', {
                        method: 'PATCH',
                        headers: {
            ...
            const updatedStage = "Approver2 Stage Completed"; // Set the stage value to "Inspection Stage Completed"
        ...
        <View style={styles.container}>
          ...
          <DataTable style={{ borderWidth: 1, borderColor: "#ccc", borderRadius: 5 }}>
            ...
            <DataTable.Header style={{ backgroundColor: "#007BFF", borderBottomWidth: 1, borderColor: "#ccc" }}>
              ...
              <DataTable.Title style={styles.headerText}>Remark by Approver2</DataTable.Title>
            </DataTable.Header>
                ...
                <DataTable.Row key={item.checklist_id || index} style={{ borderBottomWidth: 1, borderColor: "#ccc" }}>
                  ...
                  <DataTable.Cell>
                    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                      {item.remark_by_approver2 ? (
                        <>
                          <Text style={styles.valueText}>{item.remark_by_approver2}</Text>
                          <View style={{ flexDirection: "column", alignItems: "center" }}>
                            <TouchableOpacity onPress={() => handleInputModal(item, 'remark_by_approver2')}>
                              <Text style={styles.iconText}>✏️</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => handleDeleteInput(item, 'remark_by_approver2')}>
          ...
          <Modal visible={modalVisible} animationType="slide" transparent>
            <View style={styles.modalContainer}>
              <View style={styles.modalContent}>
                <TextInput
                  style={styles.input}
                  placeholder={inputType === 'remark_by_approver2' ? 'Enter Remark_by_approver2' : ''}
                  value={inputValue}
