...
router.patch('/updateApprover', async (req, res) => {
    const {remark_by_approver,checklist_id,part_id} = req.body;
  
   

    // Validate the necessary parameters
    if (!remark_by_approver||!checklist_id||!part_id) {
    ...
    try {
        ...
        const query = `
            UPDATE checklist1
            SET remark_by_approver =?
            WHERE checklist_id = ? AND part_id = ?;
        `;

        // Execute the query
        const result = await db.query(query, [remark_by_approver,checklist_id, part_id]);
